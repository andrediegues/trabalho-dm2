...
data$Number_Vehicles <- as.factor(data$Number_Vehicles)
...
```{r prepare_graphs,  echo=FALSE, message=FALSE,  warning=FALSE}
...
```{r presuure, echo=FALSE,  warning=FALSE}
...
```{r presuure2, echo=FALSE,  warning=FALSE}
...
```{r data_apriori_unique,  warning=FALSE}
data_apriori = data
data_apriori<-unique(data_apriori)
```

```{r clean_ap, echo=FALSE, message=FALSE,  warning=FALSE}
...
```{r import_subsets, echo=FALSE, message=FALSE,  warning=FALSE}
load(file="report.RData")
```

We produced some plots with the rules obtained for each cluster, under the conditions that we mentioned previously.

```{r plots_rules, message=FALSE,  warning=FALSE}
for (i in 1:length(subsets)) {
  if (length(subsets[[i]]) != 0)
    plot(subsets[[i]], method = "graph", control=list(cex=.8))
}
```

These are the 10 rules with highest lift value.

```{r top10_lift, echo=FALSE,  warning=FALSE}
...
as <- head(sort(as, decreasing = TRUE, na.last=NA, by="lift"), 10)
inspect(as)
```

After this procedure, we can conclude, from rules 1 and 2, that when the Number_Casualties is 1 (Individual) and either the Period is 2 (Morning) or the Number_Vehicles is 2 (Multiple), the Accident Severity is Slight. Although this seems normal, we were surprised to know that, if specific conditions occur, then it implies that the accident occurs at a specific week day or month as it shows in rules 3, 7 and 8. 

What was not a surprise was the fact that not many rules had pedestrians involved, since in many of the roads present in the dataset, pedestrians are not allowed to walk. Also, since the UK is a very rainy country, the fact that the speed limit, road, weather and light conditions have correlations was not a surprise either. 